        ...
        <UCard :ui="{ body: { padding: 'p-4' } }">
            ...
            <div class="space-y-4">
                <div class="flex justify-between items-end">
                    <div>
                        <div class="text-2xl font-bold">{{ node.storage_used_gb }}<span class="text-sm font-normal text-gray-500">GB</span></div>
                    </div>
                    <div class="text-right">
                         <div class="text-xs text-gray-400">of {{ node.total_available_gb }} GB</div>
                    </div>
                </div>
                <!-- Bar -->
                <div class="h-4 w-full bg-gray-200 dark:bg-gray-700 rounded-full overflow-hidden">
                    <div 
                        ...
                        :style="{ width: `${node.disk_usage || 0}%`, backgroundColor: getGaugeColor(node.disk_usage || 0) }"
                    ></div>
                </div>
                <div class="text-xs text-gray-500 text-center">
                    {{ node.disk_usage || 0 }}% Used
                </div>
            </div>
        </UCard>
